...
/**
 * `/dashboard-paper` — Phase 5.4 parity route for the daybook hero.
 *
 * Reachable at all times so Swiss and Paper views can be compared
 * side-by-side during the migration; `/dashboard` picks the branch at
 * render time based on `NEXT_PUBLIC_PAPER_UI`. Once Phase 9 deletes
 * Swiss, this route inlines into `/dashboard`.
 *
 * Queries and searchParams shape are unchanged (`range`, `from`, `to`,
 * `day`) so URLs are swappable between chromes. The Paper chrome adds
 * no new server state — everything under the ruled body composes out
 * of the same `lib/dashboard/queries` exports.
 */
export default async function DashboardPaperPage({
  searchParams,
}: {
