...
4. [x] **`/dashboard`** (overview + charts + today's entries). The daybook hero. Hero amount in oldstyle-lining figures keyed to the selected range (or drilled day), three summary figures (Entries + tally, Daily average, Top line + stamp-red `LARGEST` stamp), a pen-drawn daily bar chart with keyboard-activatable per-day drill-in + category breakdown, a ranked hand-wobble category strip, and a full `<LedgerTable>` with amend-in-place via a `<PaperClip>`-topped carbon slip. Quick-add toggles a matching pink slip at the foot of the register. Queries and `searchParams` shape unchanged (`range`, `from`, `to`, `day`); presets replace the Swiss popover calendar — custom ranges deferred to 5.5. Ships at `/dashboard-paper` (side route); `/dashboard` delegates via `PAPER_UI_ENABLED`. Swiss preserved in `_swiss.tsx`. `/design-system#dashboard` covers populated / empty / drilled-day / amend-slip on Day + Midnight.
...
*Last updated: 2026-04-21 · owner: Chien + Ledger-keeper (Claude) · Phases 0 + 1 + 2 + 3 + 4 complete · Phase 5 in progress (/login + /dashboard/recurring + /chat + /dashboard migrated; /settings + mobile pass remain).*
